...
import { Search, Filter, ChevronLeft, ChevronRight, CheckCircle, XCircle, Hotel, Clock, Loader2, ArrowLeft, ArrowRight } from 'lucide-react';
...
export default function ReceptionistBookings() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="font-serif text-2xl text-charcoal mb-1">Manage Bookings</h1>
        <p className="text-sm text-gray-500">Accept, reject, or update booking status</p>
      </div>
                      ...
                      {updatingId === booking.id ? (
                        ...
                        <div className="flex flex-wrap gap-2">
                          {nextStatuses.map((status) => (
                            ...
                            >
                              {status === 'CONFIRMED' ? 'Accept' : status === 'CANCELLED' ? 'Reject' : status.replace('_', ' ')}
                            </button>
                          ))}
                        </div>
                      ) : (
